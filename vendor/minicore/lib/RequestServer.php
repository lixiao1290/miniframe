...
class RequestServer extends Base {
	...
	public static function run() {
		if (array_key_exists ( static::class, Mini::$Mini->getConfig ( 'extentions' ) )) {
			 static::objStatic (Mini::$Mini->getConfig('extentions')[static::class]);
		}
// 		echo self::$urlDelimiter,'ijiji';
		if (1 == Mini::$Mini->getConfig ( 'routType' )) {
			...
			$rout = RequestServer::generatController ( $path );
			if ($rout ['module']) {
